...
def prediction(request):
    ...
    if request.method == "POST":
        ...
        class Project492:
            ...
            usdtry_path = 'C:/Users/Osman/Desktop/ptfprediction/USD_TRY Historical Data (7).csv'
            read_from_path = True
            target = 'PTF'
            ...
            production_base_features = ['fueloil', 'blackCoal', 'lignite', 'geothermal', 'naturalGas', 'river',
                                        'dammedHydro',
                                        'biomass', 'importCoal', 'asphaltiteCoal', 'wind', 'sun', 'importExport',
                                        'wasteheat']
        ...
        dataframe_final['is_business_time'] = np.where((dataframe_final["Time"].dt.dayofweek < 5) & (dataframe_final["hour"] >= 9) & (dataframe_final["hour"] < 18), 1, 0)
        dataframe_final['is_morning'] = np.where((dataframe_final["hour"] >= 6) & (dataframe_final["hour"] < 10), 1, 0)
        dataframe_final['is_noon'] = np.where((dataframe_final["hour"] >= 10) & (dataframe_final["hour"] < 15), 1, 0)
        dataframe_final['is_evening'] = np.where((dataframe_final["hour"] >= 15) & (dataframe_final["hour"] < 22), 1, 0)
        dataframe_final['is_night'] = np.where((dataframe_final["hour"] >= 22) | (dataframe_final["hour"] < 6), 1, 0)
        ...
        tss = TimeSeriesSplit(n_splits=3, test_size=24 * 1 * 1)
        ...
        print(df_corr.shape)
        print("correlation with MCP:")
        df_corrP = pd.DataFrame(df_corr["PTF"].sort_values(ascending=False))
        #print(df_corrP)
        ...
        cat_preds_dart = [
            (model.predict(n=pred_df.shape[0], series=y_ts, future_covariates=feature_cov_test_data)).values() for model
            in catboost_models]
        ...
        pred_df.set_index('Tarih')['PTF'].plot(ax=ax, title='Prediction')

        #plt.show()
        plt.savefig("predictionapp/static/images/figure.png")
        return render(request, "prediction.html", {"feedback": " ", "success" : "Your request is successful." })
    ...
    if request.method == "POST":
        ...
        if password == re_password:
            if User.objects.filter(email=email).exists():
                render(request, "register.html", {"error": "This email is used by another person."})
            else:
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, "login.html", {"error": "Either email or password wrong."})
